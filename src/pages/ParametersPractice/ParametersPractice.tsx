...
export default function ParametersGame() {
    ...
    const [feedback, setFeedback] = useState<"correct" | "wrong" | undefined>(undefined);
    const [givenAnswer, setGivenAnswer] = useState<"valore-basso" | "valore-normale" | "valore-alto" | undefined
    >(undefined);
    ...
    return (
        <PracticePage title="Pratica Parametri">
            <div className="flex flex-col">
                <div className="text-center border-2 border-red-200 rounded-xl p-3 mb-3 max-w-100">
                    ...
                    <div className="flex gap-3 justify-center">
                        {(
                            ...
                            <motion.div
                                key={value}
                                animate={givenAnswer === value ? { scale: 1.1 } : { scale: 1 }}
                                transition={{ type: "spring", stiffness: 400, damping: 15 }}
                            >
                                <Button
                                    onClick={() => checkAnswer(value)}
                                    disabled={!!feedback}
                                    className={givenAnswer === value ? "outline-3 outline-black" : ""}
                                >
                                    {label}
                                </Button>
                            </motion.div>
                ...
                <div className="flex justify-between gap-8">
                    <p>Risposte esatte: {rightAnswersCount}/{roundsCount}</p>
                    <NextButton
                        show={!!feedback}
                        onClick={() => {
                    ...
                    {feedback === "wrong" && (
                        ...
                        >
                            ...
                            {parameter.name === "Pressione Arteriosa (PA)" ? (
                                <>
                                    <p className="font-bold text-2xl text-center">90 &lt; PAS &lt; 150</p>
                                    <p className="font-bold text-2xl text-center">PAD &lt; 130</p>
                                </>
                            ) : (
                                <p className="font-bold text-2xl text-center">
                                    {parameter.healthyRange.min} - {parameter.healthyRange.max}
                                </p>
                            )}
                        </motion.div>
                    )}
