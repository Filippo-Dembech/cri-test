...
function OrderingGame() {
    ...
    function placeInSlot(slot: number, step: Step) {
        setPlaced((prev) => {
            ...
            if (existing) {
                setShuffled((sh) =>
                    sh.includes(existing) ? sh : [...sh, existing],
                );
            }
            next[slot] = step;
            return next;
    ...
    const score = submitted
        ? placed.filter((s, i) => s?.id === slice[i].id).length
        : 0;
    ...
    return (
        <div className="flex flex-col gap-5">
            <div className="bg-red-50 border border-red-100 rounded-xl px-4 py-3">
                <p className="text-[10px] font-semibold uppercase tracking-widest text-red-300 mb-1">
                    Istruzioni
                </p>
                <p className="text-red-700 text-xs leading-relaxed">
                    Trascina i passi negli slot nell'ordine corretto. Gli step
                    mostrati sono consecutivi all'interno della procedura.
                </p>
            </div>
                ...
                {Array(WINDOW)
                    ...
                    .map((_, i) => {
                        ...
                        return (
                            ...
                            `}
                            >
                                <span className="text-[11px] text-red-300 font-bold w-4 shrink-0">
                                    {i + 1}
                                </span>
                                {p ? (
                                    <div className="flex items-center justify-between flex-1 gap-2">
                                        ...
                                        {submitted && (
                                            <span
                                                className={`text-xs font-bold shrink-0 ${isCorrect ? "text-green-600" : "text-red-500"}`}
                                            >
                                                {isCorrect ? "✓" : "✗"}
                                            </span>
                                        )}
                                    ...
                                    <span className="text-red-200 text-xs italic">
                                        trascina qui un passo
                                    </span>
                                )}
                            </div>
                        );
            ...
            <AnimatePresence>
                {submitted && (
                    ...
                    >
                        <p
                            className={`text-xs font-semibold ${score === WINDOW ? "text-green-700" : "text-amber-700"}`}
                        >
    ...
    return shuffle(
        STEPS.slice(0, -1).map((step, i) => {
            ...
            const distractors = shuffle(
                STEPS.filter((s) => s.id !== correct.id && s.id !== step.id),
            ).slice(0, 2);
            return {
                prompt: step,
                correct,
        ...
        return (
            ...
            >
                ...
                <p className="text-red-900 font-semibold text-lg">
                    {correct} su {results.length} corrette
                </p>
                <p className="text-red-400 text-sm text-center">
                    {pct >= 80
                        ? "Ottima conoscenza della sequenza."
                        : pct >= 50
                          ? "Buon lavoro — ripassare i passaggi centrali."
                          : "Studia la tecnica e riprova."}
                </p>
                <motion.button
                    onClick={restart}
                    className="w-full bg-red-500 text-white py-2.5 rounded-xl font-semibold text-sm cursor-pointer hover:bg-red-600 transition-colors"
                        ...
                        {q.options.map((opt) => {
                            ...
                            if (isAnswered) {
                                if (correct)
                                    ...
                                    style =
                                        "opacity-40 bg-white border-red-100 text-red-300";
                            }
                            return (
                                <motion.button
                                    key={opt.id}
                    ...
                    <AnimatePresence>
                        {isAnswered && (
                            ...
                            >
                                ...
                                >
                                    <p
                                        className={`text-xs font-semibold ${isCorrect ? "text-green-700" : "text-red-500"}`}
                                    >
                                        {isCorrect
                                            ? "Corretto"
                                            : `Il passo corretto era: "${q.correct.text}"`}
                                    </p>
                                    {q.correct.detail && (
